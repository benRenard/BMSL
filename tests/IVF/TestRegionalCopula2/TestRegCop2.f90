...
filenumber=45_mik
inFile="C:\Mydoc\Forge\hydrostat\trunk\Fortran\test\Tests_IVF\TestRegionalCopula2\Data\"

do loop=1,1 !1,filenumber
...
distId=GEV
DistName='GEV'
OutFile='C:\Mydoc\Forge\hydrostat\trunk\Fortran\test\Tests_IVF\TestRegionalCopula2\MCMC\'
...
if(associated(SpatPar(3)%SpatialPar)) deallocate(SpatPar(3)%SpatialPar)
allocate(SpatPar(3)%SpatialPar(1))
...
call InitializeSpatialpar(Name=SpatName, SpatReg='Linear_L1', SpatialPar=SpatPar(3)%SpatialPar(1),s_cov=s_cov(1:1,:),err=err, mess=mess)
If(err>0) then
    mess='MeanProg error: '//trim(mess)
    write (*,*) mess
    read (*,*)
...
if(associated(Index_regionalPar(3)%Indice)) deallocate(Index_regionalPar(3)%Indice)
allocate(Index_regionalPar(3)%Indice(1))
Index_regionalPar(3)%Indice=(/1_mik/)
!###############
!################################################################################## 
...
PL(1)%par=(/25._mrk,25.001_mrk/)
!PL(3)%par=(/10._mrk,10.001_mrk/)
!pL(4)%par=(/-0.1001_mrk,-0.1_mrk/)
PL(4)%dist=GAUSS
PL(4)%par=(/0._mrk,50._mrk/)
...
call RegMCMC(X=x, Model=Model, Dpar=Dpar, Copula=Copula, Covariates=allcov, PriorList=PL,&
            ...
            UseRFortran=.false.,&
            OutFile=trim(OutFile)//"SimulationCopTemp"//trim(number_string(loop))//".txt", &
            ! error handling
            err=err,mess=mess)
