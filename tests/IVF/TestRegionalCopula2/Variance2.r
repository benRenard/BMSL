...
folderTest="hydrostat/trunk/Fortran/test/Tests_IVF/TestRegionalCopula2/"
a<-c()
b<-c()
ha<-c()
la<-c()
...
plot(phi$x,a,type='l',main="GEV",ylab="Variance",xlab=expression(phi),cex.main=2,cex.lab=2,cex.axis=1.5,lwd=2)
...
x11(20,10)
par(mar=c(5,5,4,2) + 0.1)
plot(phi$x,ha,ylim=c(0.05,0.35),main="GEV",ylab="Value",xlab=expression(phi),cex.main=2,cex.lab=2,cex.axis=1.5,lwd=2)
points(phi$x,la)
...
for (k in 1:20){
    ...
    phibox1[(5000*(k-1)+1):(5000*k),2]=mcmc$par5[15001:20000]
}
...
for (k in 1:15){
    ...
    phibox2[(5000*(k-1)+1):(5000*k),2]=mcmc$par5[15001:20000]
}
...
for (k in 1:10){
    ...
    phibox3[(5000*(k-1)+1):(5000*k),2]=mcmc$par5[15001:20000]
}
